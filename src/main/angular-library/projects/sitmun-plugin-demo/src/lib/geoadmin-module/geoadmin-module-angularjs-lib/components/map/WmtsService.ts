  ...
  module.provider('gaWmts', function() {
    this.$get = function(gaDefinePropertiesForLayer, gaMapUtils, gaUrlUtils,
      ...
      var createWmtsLayer = function(options) {
		...
		//FIXME the attribution is defined in the source configuration
        if (options.attribution && options.sourceConfig) {
          options.sourceConfig["attributions"] = options.attribution;
        }
		...
		//FIXME set new property
        layer.setProperties({id: 'WMTS||' + options.layer + '||' + options.capabilitiesUrl});
        ...
        */
	   //FIXME set new layer properties
       layer.setProperties({useThirdPartyData:
            gaUrlUtils.isThirdPartyValid(options.sourceConfig.urls[0])});
        layer.setProperties({label: options.label});
        layer.setProperties({url: options.capabilitiesUrl});
        var timestamps = options.timestamps;
        layer.setProperties({timestamps: timestamps});
        layer.setProperties({timeEnabled: (timestamps && timestamps.length > 1)});
        if (timestamps && timestamps.length === 2 &&
              timestamps[0] === 'current') {
                layer.setProperties({timeEnabled: false});
        } else {
          layer.setProperties({timeEnabled: (timestamps && timestamps.length > 1)});
        }
        /*
