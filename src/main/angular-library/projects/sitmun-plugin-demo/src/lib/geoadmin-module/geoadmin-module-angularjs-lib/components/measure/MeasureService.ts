    ...
    this.$get = function($document, measureFilter, gaMapUtils, gaGeomUtils) {
      var Measure = function() {
        ...
        this.getAzimuth = function(geom) {
          ...
          if (geom instanceof ol.geom.Polygon) {
			//FIXME retrive the geometry point coordinates
            var pGeom:ol.geom.Polygon = geom;
            var coordList = pGeom.getCoordinates();
            coords = coordList[0];
          }
          pt1 = coords[0];
          pt2 = coords[1];
        ...
        this.registerOverlaysEvents = function(map, layer) {
          ...
          layer.getSource().on('removefeature', function(evt) {
			//FIXME retrieve the feature object
		  ...
		  //FIXME check the property displayInLayerManager
          if (!layer.getProperties() ||
              !layer.getProperties()["displayInLayerManager"]) {
            return;
          }
