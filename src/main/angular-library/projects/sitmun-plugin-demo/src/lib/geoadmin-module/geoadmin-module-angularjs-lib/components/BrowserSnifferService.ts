...
var module = angular.module('ga_browsersniffer_service', [
  ...
  module.provider('gaBrowserSniffer', function() {
    ...
    this.$get = function($window, gaPermalink) {
	  ...
	  //FIXME evaluate correctly which browser is in use
      var isMsie = false;
      if (isNaN(msie)) {
      ...
      var isIos = !isMsie &&  /(iPhone|iPad|iPod)/.test(ua);
      var ios = null;
      if (isIos) {
        ios = +((/\((iPhone|iPad|iPod).+OS (\d{1,2})_/.exec(ua) || [])[2]);
      }
      var android = /(android)/i.test(ua);
      var webkit = !isMsie && /WebKit/.test(ua);
      var opera = !isMsie && /(OPiOS|OPR)\//.test(ua);
      var chrome = !isMsie && !opera && /(CriOS|Chrome)\//.test(ua);
      var safari = !isMsie && !opera && !chrome && /Safari/.test(ua);
      var mobile = /\/mobile\.html$/.test($window.location.pathname);
      //TODO FIXME use a better way to detect mobile capabilities
      mobile = mobile || $window.useMobileProperties;
      ...
      if (isMsie && (msie > 9)) {
        ...
        //$('body').find('input[type=range]').on('change', function() {
          $(this).trigger('input');
        });
      }
