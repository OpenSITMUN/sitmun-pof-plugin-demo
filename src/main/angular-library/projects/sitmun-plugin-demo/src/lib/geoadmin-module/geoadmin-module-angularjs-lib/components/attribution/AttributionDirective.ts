...
import * as angular from 'angular';
  ...
  module.directive('gaAttribution', function($translate, $window,
      gaAttribution, $rootScope, gaDebounce, gaEvent, gaGlobalOptions) {
    return {
      restrict: 'A',
      scope: {
      ...
      link: function(scope, element, attrs) {
        ...
        var update = function(element, layers) {
          if (gaGlobalOptions.disableDisplayActiveLayerAttribution) {
            //Do not show the layer attribution data
            return;
          }
          var attrs = {}, list = [], is3dActive = scope.is3dActive();
          layers = layers?layers:new Array();
          if (is3dActive) {
        ...
        scope.$watchCollection('layers | filter:layerFilter', function(layers) {
          if (!layersFiltered && (!layers || !layers.length)) {
			//Do nothing if no layers have been loaded
            return;
          }
