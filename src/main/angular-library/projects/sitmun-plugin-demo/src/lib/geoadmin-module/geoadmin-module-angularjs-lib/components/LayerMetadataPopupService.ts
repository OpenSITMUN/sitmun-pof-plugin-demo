      ...
      var LayerMetadataPopup = function() {
        ...
        var create = function(layer) {
          ...
          $rootScope.$on('$translateChangeEnd', function(evt, newLang) {
            ...
            updateContentLang(popup, layer, newLang, null);
          });
        };

		//Check if it has a metadata url defined
		//FIXME define the function variable
        var hasMetadataUrlAvailable = function(layer) {
          if (layer.bodId) {
            var url = gaLayers.getMetadataUrlOfLayer(layer.bodId);
            return (url != null) &&
                  (url != undefined) &&
                  (url != "");
          } else { 
            return gaMapUtils.isExternalWmsLayer(layer);
          }
        }
        ...
        this.toggle = function(olLayerOrBodId) {
          ...
          if (popup) { // if the popup already exist we toggle it
            ...
            if (hasMetadataUrlAvailable(layer)) {
              create(layer);
            }
          }
        };
