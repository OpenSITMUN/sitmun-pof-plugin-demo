    ...
    this.$get = function($http, $q, $rootScope, $translate, $window,
      ...
      var Layers = function(
        ...
        var getLayersConfigUrl = function(lang) {
		  //Returns the current language layer configuration file url
          if (layersConfigUrl != null) {
        ...
        var getMetaDataUrl = function(layer, lang) {
		  //Returns the current language layer metadata file url
          var url = legendUrl;
          if (layers && layers[layer] && 
              (layers[layer].legendUrl != undefined)) {
            url = layers[layer].legendUrl;
          }
          if (url) {
            url = url.
                replace('{Layer}', layer).
        ...
        // Load layers config
        var lastLangUsed;
		//Load the layer configuration, removing or updating the layer loaded list
        var loadLayersConfig = function(lang, layerTypeToUpdate) {
          //Layer type to update
          // 0: remove all layers before updating
          // 1: remove only tree layers before updating
          // 2: remove only background layers before updating
          layerTypeToUpdate = ((layerTypeToUpdate == null) || 
                               (layerTypeToUpdate == undefined))?0:layerTypeToUpdate;
          var parseResponse = function(response) {
            ...
            if (layers && (layerTypeToUpdate != 0)) {
              // Remove the unwanted layers
              var layersToDelete = [];
              for (var i in layers) {
                if (((layerTypeToUpdate == 2) && (layers[i].background)) || 
                    ((layerTypeToUpdate == 1) && (!layers[i].background))) {
                  layersToDelete.push(i);
                }
              }
			  //FIXME use a new index variable
              for (var idx = 0, iLen = layersToDelete.length; idx < iLen; idx++) {
                delete layers[layersToDelete[idx]];
              }
            }
            ...
            if (!layers) { // First load
              ...
              $rootScope.$on('$translateChangeEnd', function(event, newLang) {
                loadLayersConfig(newLang.language, layerTypeToUpdate);
              });
            } else { // Only translations has changed
              if (layerTypeToUpdate == 0) {
                layers = response.data;
              } else {
                angular.extend(layers, response.data);
              }
		  ...
		  //Retrieve the layer configuration information from an object list or a remote file
          if (tempConfiguration) {
            var promise = new Promise(function(resolve, reject) {
                if ((typeof tempConfiguration).toLowerCase() == "object") {
                  resolve({
                            data: tempConfiguration
                          });
                  tempConfiguration = null;
                } else {
                  reject(Error("Not a valid layers configuration loaded"));
                }
            }).then(parseResponse);
            return promise;
          } else {
		...
		//FIXME loadLayersConfig has 2 parameters
        var configP = loadLayersConfig(gaLang.get(), null);
		...
		//Load the layers configuration
        this.loadLayersConfiguration = function(layersConfig) {
          tempConfiguration = layersConfig;
		//FIXME loadLayersConfig has 2 parameters
          configP = loadLayersConfig(gaLang.get(), null);
        }

		//Load tree layers configuration only (the currently loaded will be removed)
        this.loadTreeLayersConfiguration = function(layersConfig) {
          tempConfiguration = layersConfig;
          return loadLayersConfig(gaLang.get(), 1);
        }

		//Load background layers configuration only (the currently loaded will be removed)
        this.loadBackgroundLayersConfiguration = function(layersConfig) {
          tempConfiguration = layersConfig;
          return loadLayersConfig(gaLang.get(), 2);
        }
            ...
            if (!olSource) {
              ...
              olSource = config.olSource = new ol.source.WMTS({
                ...
                crossOrigin: crossOrigin,
		  		//FIXME add mandatory parameters
            ...
            if (config.singleTile === true) {
              if (!olSource) {
                olSource = config.olSource = new ol.source.ImageWMS({
		  		  //FIXME getWmsTpl has 2 parameters
                  url: getImageryUrls(getWmsTpl(srcUrl, {}), wmsSubdomains)[0],
                  params: wmsParams,
                  crossOrigin: crossOrigin,
                  ratio: 1,
				  //FIXME add mandatory paramter
                  projection: null
                });
              }
              ...
              if (!olSource) {
                olSource = config.olSource = new ol.source.TileWMS({
		  		  //FIXME getWmsTpl has 2 parameters
                  urls: getImageryUrls(getWmsTpl(srcUrl, {}), wmsSubdomains),
        ...
        /**
         * Get Metadata url of given layer bodId
         * Uses current topic and language
         * Returns a promise. Use accordingly
         */
        this.getMetadataUrlOfLayer = function(bodId) {
          return getMetaDataUrl(bodId, gaLang.get());
        }
