...
import * as ol from 'openlayers';
  ...
  module.directive('gaLayermanager', function($compile, $timeout,
    ...
    var win = $($window);
	//FIXME createPopover may have 3 parameters
    var createPopover = function(bt, element, scope?) {
      ...
      '<li ng-repeat="layer in filteredLayers" class="ga-layer-folded"' +
        ...
        '<div class="ga-layer-infos ga-grab">' +
          ...
          '<label class="ga-checkbox" title="{{layer.label}}">' +
            ...
            '<span></span>' +
            '{{layer.label}}' +
            '<span ng-if="isQueryable(layer)" class="ga-icon ga-btn fa fa-info-sign"></span>' +
          '</label>' +
    ...
    return {
      ...
      link: function(scope, element, attrs) {
        ...
        scope.enableDragAndDrop = function() {
          ...
          if (!slip) {
			//FIXME check if the plugin/library is loaded/defined before trying to use it
            if ($window.Slip) {
              slip = new $window.Slip(list);
            }
          } else {
            slip.attach(list);
          }
        ...
        scope.duplicateLayer = function(evt, layer) {
		  ...
		  //Notify the layer duplication the event will be propagated to the map component
          this.$emit("gaDisplayedDuplicated", [dupLayer, index]);
          evt.preventDefault();
        };
        ...
        scope.moveLayer = function(evt, layer, delta) {
		  ...
		  //Notify the layer modification it will be propagated to the map component
          this.$emit("gaDisplayedMoved", [layer, insertIndex]);
          evt.preventDefault();
        };

        scope.removeLayer = function(layer) {
          map.removeLayer(layer);
		  //Notify the layer removal it will be propagated to the map component
          this.$emit("gaDisplayedRemoved", layer);
        };
