  ...
  module.provider('gaPermalinkLayersManager', function() {
    ...
    this.$get = function($rootScope, gaLayers, gaPermalink, $translate,
      ...
      return function(map) {
        ...
        var addTopicSelectedLayers = function() {
          ...
          if (topic.plConfig) {
            ...
            addLayers(p.layers ? p.layers.split(',') : [],
                ...
                p.layers_style ?
                  p.layers_style.split(',') : undefined
            );
          } else {
		  	//FIXME define all mandatory parameters
            addLayers(topic.selectedLayers.slice(0).reverse(), null, 
                      null, null, null, null);
            var activatedLayers = topic.activatedLayers;
            if (activatedLayers.length) {
              addLayers(activatedLayers.slice(0).reverse(), null, 
              		null, null, null, null);
            }
          }
        };
          ...
          angular.forEach(layerSpecs, function(layerSpec, index) {
            ...
            if (bodLayer) {
              ...
              if (angular.isDefined(layer)) {
                ...
                map.addLayer(layer);
                if (scope) {
                  scope.$emit("gaDisplayedAdded", [layer, visible]);
                }
              }
        ...
        function updateTopicAndLayersConfiguration() {
		  //Update the topics information
          var topicP = gaTopic.loadConfig();
          var layerP = gaLayers.loadConfig();
          var pArray = [];
          if (topicP != null) {
            pArray.push(topicP);
          }
        ...
        updateTopicAndLayersConfiguration();
		//Listen to topics information changes (the respective directive property has been altered by the parent app)
