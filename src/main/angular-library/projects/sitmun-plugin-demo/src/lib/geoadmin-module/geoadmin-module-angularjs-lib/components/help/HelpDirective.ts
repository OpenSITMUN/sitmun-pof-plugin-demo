...
var module = angular.module('ga_help_directive', [
  ...
  */
  module.directive('gaHelp', function(gaGlobalOptions) {
    return {
      restrict: 'A',
      scope: {
        helpIds: '@gaHelp',
      ...
      template: 
      '<button class="ga-btn ga-icon fa fa-question-circle"' +
              ' ga-help-action="{{helpIds}}">' +
      '</button>',
      link: function(scope, element, attrs) {
        scope.options = scope.optionsFunc();
		//Check if the display of the help tool is disabled globally
        if (!gaGlobalOptions.helpInfoDisabled) {
  ...
  module.directive('gaHelpAction', function($rootScope, $sce, gaHelp,
      gaPopup, $window, gaGlobalOptions) {
        ...
        var displayHelp = function(evt) {
          if (angular.isDefined(popup)) {
            ...
            var updateContent = function(doOpen) {
			  ...
			  //Retrieve the help information from a service or a url template
              if (gaGlobalOptions.helpUrlTemplate) {
                gaHelp.get(0).then(function(res) {
                  for (i = 0; i < len; i++) {
                    if (res[ids[i]]) {
                      results.push([
                        $sce.trustAsHtml(res[ids[i]].title),
                        $sce.trustAsHtml(res[ids[i]].content),
                        res[ids[i]].image
                      ]);
                      resultReceived();
                    }
                  ...
                  var msg = 'No help found for id ' + ids[0];
                  $window.alert(msg);
                });
              } else {
                for (i = 0; i < len; i++) {
